...
describe Dessert do
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("brownie", "tons", chef) }.to raise_error("Amount must be a number")
    end
  end
  ...
  describe "#eat" do
    ...
    it "raises an error if the amount is greater than the quantity" do
      expect { brownie.eat(60)}.to raise_error("Not enough brownies")
    end
  end

  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Lakhte the God of Programming")
      expect(brownie.serve).to eq("Lakhte made 50 brownies!")
    end
  end
